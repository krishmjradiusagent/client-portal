...
export interface PinnedListProps {
  ...
  className?: string;
  variant?: "pinned" | "actions";
  onItemClick?: (item: PinnedListItem) => void;
}
...
function ItemCard({
  ...
  const isAction = variant === "actions";
  ...
  if (isAction) {
    return (
      <button
        ...
        className={cn(
          "flex min-h-11 w-fit max-w-[300px] cursor-pointer items-center gap-3 rounded-2xl border border-white/10 bg-zinc-950/90 px-4 py-2.5 text-zinc-300 shadow-md shadow-black/15 backdrop-blur-xl hover:border-white/20 hover:bg-zinc-900/95",
        )}
      >
        <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-xl bg-white/10 text-zinc-300">
          {item.icon}
        </span>
        <span className="min-w-0 flex-1 text-left">
          <span className="block truncate text-[14px] font-medium leading-tight text-zinc-100">
            {item.name}
          </span>
        </span>
      </button>
  ...
  return (
    <motion.div
      ...
      exit="exit"
      className={cn(
        "flex items-center gap-3 rounded-2xl border border-transparent bg-muted px-3 py-3.5 text-muted-foreground/50",
        pinned && " bg-blue-100 dark:bg-blue-950/30",
      )}
    >
      <div
        className={cn(
          "flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-background text-foreground/70",
        )}
      >
        {item.icon}
      </div>
...
export function PinnedList({ items, className, variant = "pinned", onItemClick }: PinnedListProps) {
  ...
  if (variant === "actions") {
      return (
        <LayoutGroup>
        <div className={cn("flex w-fit max-w-[300px] flex-col items-end gap-1.5", className)}>
            {items.map((item) => (
              <ItemCard
                key={item.id}
                item={item}
